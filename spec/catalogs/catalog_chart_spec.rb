...
describe CatalogChart do
  ...
  describe "#score_coordinates" do
    ...
    let(:coordinates) { chart.score_coordinates }
    ...
    it "should have x values sorted chronologically" do
      Timecop.freeze do
        ...
        expect(chart.score_coordinates.count).to eq 4
        expect(Time.at(chart.score_coordinates.first[:x]).to_date).to eq oldest_entry.date
      end
    end
    ...
    it "strips entries without scores from coordinates" do
      ...
      expect(chart.score_coordinates.count).to eq 2
    end
    ...
    it "should return a limit chart for a limited date range" do
      expect(chart.score_coordinates(Date.yesterday, Date.today).count).to eq 2
    end
